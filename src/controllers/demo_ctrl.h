...
private:
     ...
     std::vector<float> m_lutDistance; // range distances for each gene

     // initialize our lookup tables with random values
     void initLUT();

     // get a lut index from a distance (in mm)
     size_t getLUTIndex(float distance) const;

     // calculate the local performance
     // power-law: a*(x+1)^b.
     int calcPerformance(uint8_t distance, uint8_t cluster) const;
};
