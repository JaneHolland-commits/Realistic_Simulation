    ...
    for (uint32_t i = 0; i < m_entities.size(); ++i) {
        ...
        for (int posTrial = 0; posTrial < maxPosTrial; ++posTrial) {
            ...
            if (MoveEntity(m_entities[i]->GetEmbodiedEntity(), position, orientation, false)) {
                objAdded = true;
                stPos << position.GetX() << " " << position.GetY() << "\n";
                break;
            }
        }
...
void AbstractGALoopFunction::PostExperiment()
{
   ...
   // orientation.FromEulerAngles(zAngle, CRadians::ZERO, CRadians::ZERO); // z, y, x
    ...
    std::ofstream edPos;
    edPos.open("endPosition.txt", std::ofstream::app);

    std::ofstream gen;
    gen.open("generation.txt", std::ofstream::app);

    LOG << "Generation " << m_iCurGeneration << "\t"
        << getGlobalPerformance() << std::endl;

    gen << getGlobalPerformance() << "\n";

    for (uint32_t i = 0; i < m_entities.size(); ++i) {

	//if (m_entities[i]->GetEmbodiedEntity(), position, orientation, true) {

         edPos << m_entities[i]->GetEmbodiedEntity().GetOriginAnchor().Position.GetX() << " " << m_entities[i]->GetEmbodiedEntity().GetOriginAnchor().Position.GetY() << "\n";
	//}
    }
